...
Adress field below -->
    ...
    <div class="container_fluid">
        <input id="address" name="{{$field['name']}}[]" class="form-control input-md" placeholder="Address"  value="{{$field_values->address or ''}}" >
        <div id="map_canvas" style="width:100%; height:300px"></div>
        <br>
        <div class="row">
            <div class="col-md-3">
                <input id="latitude" name="{{$field['name']}}[]"  type="hidden" value="{{$field_values->lattitude or ''}}" >
            </div>
            <div class="col-md-3">
                <input id="longitude" name="{{$field['name']}}[]"  type="hidden" value="{{$field_values->longitude or ''}}">
            </div>
        </div>
    </div>
...
<script type='text/javascript' src="https://maps.googleapis.com/maps/api/js?key={{setting('iplaces::api')}}&extension=.js&output=embed"></script>
        ...
        $(function() {
            $("#address").autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    address_values = {"address":$("#address").val(),"longitude":$("#longitude").val(),"lattitude":$("#latitude").val()};

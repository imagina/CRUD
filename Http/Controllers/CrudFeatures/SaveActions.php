...
namespace Modules\Bcrud\Http\Controllers\CrudFeatures;
...
trait SaveActions
{
    ...
    {
        $saveAction = session('save_action', config('bcrud.backpack.crud.default_save_action', 'save_and_back'));
        $saveOptions = [];
        $saveCurrent = [
            'value' => $saveAction,
    ...
    public function setSaveAction($forceSaveAction = null)
    {
        if ($forceSaveAction) {
            ...
            $saveAction = \Request::input('save_action', config('bcrud.backpack.crud.default_save_action', 'save_and_back'));
        }
    ...
    private function getSaveActionButtonName($actionValue = 'save_and_back')
    {
        switch ($actionValue) {
            case 'save_and_edit':
                return trans('bcrud::crud.save_action_save_and_edit');
                break;
            case 'save_and_new':
                return trans('bcrud::crud.save_action_save_and_new');
                break;
            case 'save_and_back':
            default:
                return trans('bcrud::crud.save_action_save_and_back');
                break;
        }
    }
}

    ...
    */
    ...
    public function getCellView($column, $entry)
    {
        if (! isset($column['type'])) {
            return \View::make('bcrud::columns.text')->with('crud', $this)->with('column', $column)->with('entry', $entry)->render();
        } else {
            if (view()->exists('vendor.backpack.crud.columns.'.$column['type'])) {
                ...
                if (view()->exists('bcrud::columns.'.$column['type'])) {
                    return \View::make('bcrud::columns.'.$column['type'])->with('crud', $this)->with('column', $column)->with('entry', $entry)->render();
                } else {
                    return \View::make('bcrud::columns.text')->with('crud', $this)->with('column', $column)->with('entry', $entry)->render();
                }
            }
